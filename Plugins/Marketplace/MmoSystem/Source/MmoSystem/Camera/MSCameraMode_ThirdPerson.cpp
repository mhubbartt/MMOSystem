...
#include "MSCameraMode_ThirdPerson.h"
#include "MSCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "MSPenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "MSCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(MSCameraMode_ThirdPerson)

namespace MSCameraMode_ThirdPerson_Statics
{
...
UMSCameraMode_ThirdPerson::UMSCameraMode_ThirdPerson()
{
	...
	PenetrationAvoidanceFeelers.Add(FMSPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FMSPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FMSPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FMSPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FMSPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FMSPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FMSPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UMSCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UMSCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UMSCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UMSCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IMSCameraAssistInterface* TargetControllerAssist = Cast<IMSCameraAssistInterface>(TargetController);

	IMSCameraAssistInterface* TargetActorAssist = Cast<IMSCameraAssistInterface>(TargetActor);
	...
	IMSCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IMSCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IMSCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IMSCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UMSCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO ICameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FMSPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(MSCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UMSCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UMSCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
